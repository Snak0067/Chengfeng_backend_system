...
folder = 'data/raw_data/train'  # 'train', 'test'
npy_folder = 'data/npy3/train'  # 'train_npy/npy3', 'test_npy/npy3'
out_folder = 'data/frame/train_frame_data'  # 'train_frames' 'test_frames'
    ...
    for i in tqdm(range(len(files))):
        ...
        if 'color' in name:
            try:
                ...
                # 如果文件不存在，则跳过该文件并继续执行程序
                print(f"文件{os.path.join(npy_folder, name + '.npy')}不存在，跳过。")
                continue
            except IndexError:
                # 如果视频格式损坏，则跳过该文件并继续执行程序
                print(f"文件{os.path.join(npy_folder, name + '.npy')}'s moov atom not found，跳过。")
                continue
